...
═══════════════════════════════════════ */
const state = {
    method: 'card',       // 'card' | 'qr' | 'wallet' | 'cash'
    promo: null,          // applied promo { code, discount }
    usePoints: false,
    baseAmount: 0,        // set from URL/session
    vatRate: 0.10,
    timerSec: 14 * 60 + 59,
    timerInterval: null,
    // Demo booking data (real app: read from sessionStorage / Thymeleaf model)
    booking: {
        roomName: 'Phòng Deluxe',
        roomBadge: 'Phổ biến',
        roomImg: 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=400&q=80',
    ...
    loyaltyValue: 125000,
    promoCodes: {
        'HOTEL10': { discount: 0.10, label: 'Giảm 10%' },
        'SUMMER20': { discount: 0.20, label: 'Giảm 20% Hè 2026' },
        'NEWGUEST': { discount: 0.15, label: 'Khách mới giảm 15%' },
    }
};
...
function initSummary() {
    ...
    // Room
    $('summaryRoomName').textContent = b.roomName;
    $('summaryRoomBadge').textContent = b.roomBadge;
    if ($('summaryRoomImg')) $('summaryRoomImg').src = b.roomImg;

    // Dates & guests
    $('summaryCheckin').textContent = b.checkin;
    $('summaryCheckout').textContent = b.checkout;
    $('summaryNights').textContent = `${b.nights} đêm`;
    $('summaryGuests').textContent = `${b.adults} người lớn${b.children ? `, ${b.children} trẻ em` : ''}`;
    $('summaryRooms').textContent = `${b.rooms} phòng`;

    // Guest recap
    $('recapName').textContent = b.guestName;
    $('recapPhone').textContent = b.guestPhone;
    $('recapEmail').textContent = b.guestEmail;

    // Prices
    state.baseAmount = b.pricePerNight * b.nights * b.rooms;
    renderPrices();

    // QR
    $('qrRef').textContent = `HTH-${new Date().toISOString().slice(0,10).replace(/-/g,'')}-${String(Math.floor(Math.random()*9999)+1).padStart(4,'0')}`;
    $('bookingCode').textContent = $('qrRef').textContent;
}
...
function renderPrices() {
    ...
    let discount = 0;
    if (state.promo) {
        discount = Math.round(subtotal * state.promo.discount);
        $('rowDiscount').style.display = 'flex';
        $('priceDiscount').textContent = `−${fmt(discount)}`;
        subtotal -= discount;
    } else {
    ...
    $('pricePer').textContent = `${fmt(b.pricePerNight)} × ${b.nights} đêm`;
    $('priceVat').textContent = fmt(vat);
    $('totalAmount').textContent = fmt(total);
    $('qrAmount').textContent = fmt(total);
    $('btnPayAmount').textContent = fmt(total);

    // Success modal total
    $('sucTotal').textContent = fmt(total);
    $('sucRoom').textContent = b.roomName;
    $('sucCheckin').textContent = b.checkin;
    $('sucCheckout').textContent = b.checkout;
}
...
function initTabs() {
    $$('.method-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            $$('.method-panel').forEach(p => p.classList.remove('active'));
            document.getElementById(`panel${capitalize(method)}`).classList.add('active');
...
function updatePayBtnLabel() {
    ...
    $('btnPayText').textContent = labels[state.method] || 'Xác nhận thanh toán';
}
...
function initCardForm() {
    ...
    numInput.addEventListener('input', () => {
        let v = numInput.value.replace(/\D/g,'').slice(0,16);
        numInput.value = v.replace(/(.{4})/g,'$1 ').trim();
        $('cardNumberDisplay').textContent = formatDisplay(v, '•••• •••• •••• ••••', 4);
        detectCardType(v);
        clearError('errCardNumber');
    });
    ...
    holderInput.addEventListener('input', () => {
        const v = holderInput.value.toUpperCase();
        holderInput.value = v;
        $('cardHolderDisplay').textContent = v || 'HỌ VÀ TÊN';
        clearError('errCardHolder');
    });
...
function formatDisplay(raw, template, groupSize) {
    if (!raw) return template;
    let display = '';
    let idx = 0;
    for (let ch of template) {
        if (ch === '•') {
            display += idx < raw.length ? raw[idx++] : '•';
        } else {
            display += ch;
        }
    }
...
function initCvvHelp() {
    ...
    document.addEventListener('click', e => {
        if (!btn.contains(e.target)) tip.hidden = true;
    });
}
...
function initWallet() {
    $$('.wallet-radio').forEach(radio => {
        radio.addEventListener('change', () => {
            ...
            const labels = { momo: 'MoMo', zalopay: 'ZaloPay', vnpay: 'VNPay', shopee: 'ShopeePay' };
            $('selectedWalletName').textContent = labels[name] || name;
            $('walletRedirectInfo').hidden = false;
        });
    });
...
function applyPromo() {
    ...
    if (state.promoCodes[code]) {
        state.promo = { code, ...state.promoCodes[code] };
        msg.textContent = `✓ Áp dụng thành công: ${state.promo.label}`;
        msg.className = 'promo-msg success';
        $('promoInput').disabled = true;
        $('btnPromo').textContent = 'Xóa';
        $('btnPromo').onclick = removePromo;
        renderPrices();
        showToast(`Mã "${code}" đã được áp dụng!`, 'success');
    } else {
...
function startTimer() {
    ...
    state.timerInterval = setInterval(() => {
        ...
        if (state.timerSec <= 0) {
            ...
            $('qrExpire').textContent = '00:00';
            $('timerBanner').style.background = '#fef2f2';
            $('timerBanner').style.borderColor = '#fca5a5';
            showToast('Phiên thanh toán đã hết hạn. Vui lòng tải lại trang.', 'error');
...
function renderTimer() {
    ...
    const s = String(state.timerSec % 60).padStart(2, '0');
    const txt = `${m}:${s}`;
    $('countdownTimer').textContent = txt;
    $('qrExpire').textContent = txt;
}
...
function initPayButton() {
    ...
    $('btnDownloadInvoice')?.addEventListener('click', () => {
        showToast('Đang tải hóa đơn PDF…', 'success');
    });
}
...
function clearError(errId) {
    $(errId).textContent = '';
    const inputId = errId.replace('err', '').replace(/^./, c => c.toLowerCase());
...
function showProcessing() {
    ...
    // Animate steps
    const steps = ['procStep1', 'procStep2', 'procStep3'];
    steps.forEach(id => {
        $(id).classList.remove('done', 'active');
        $(id).querySelector('.proc-dot').style = '';
    });
...
function showSuccess() {
    clearInterval(state.timerInterval);
    renderPrices(); // refresh modal values
...
function showToast(message, type = 'default') {
    ...
    const icons = {
        success: `<svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="10" cy="10" r="8" fill="#f0fdf4" stroke="#22c55e" stroke-width="1.5"/><path d="M6.5 10l2.5 2.5 4.5-5" stroke="#22c55e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
        error: `<svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="10" cy="10" r="8" fill="#fef2f2" stroke="#dc2626" stroke-width="1.5"/><path d="M10 6v5M10 13v1" stroke="#dc2626" stroke-width="1.5" stroke-linecap="round"/></svg>`,
        default: `<svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="10" cy="10" r="8" fill="#fff7f3" stroke="#e55200" stroke-width="1.5"/><path d="M10 9v5M10 6.5v1" stroke="#e55200" stroke-width="1.5" stroke-linecap="round"/></svg>`,
    };
